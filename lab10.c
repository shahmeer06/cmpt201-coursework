...
1. How is the client sending data to the server? What protocol?
   The client is using TCP (SOCK_STREAM) to send data to the server via a socket
connection.

2. What data is the client sending to the server?
   The client sends 5 messages: "Hello", "Apple", "Car", "Green", and "Dog".
Each message is sent as a 1024-byte buffer (BUF_SIZE) with one message per
second.

Understanding the Server:
1. Explain the argument that the `run_acceptor` thread is passed as an argument.
   The argument is a pointer to a struct acceptor_args which contains: a run
flag (atomic bool) to control the thread's execution, a pointer to the
list_handle for storing messages, and a pointer to the list_lock mutex for
thread-safe list access.

2. How are received messages stored?
   Messages are stored in a linked list structure. Each message is stored in a
list_node which contains a pointer to the next node and a pointer to the message
data. The list_handle maintains a pointer to the last node and a count of total
messages.

3. What does `main()` do with the received messages?
   Main waits until all expected messages are received (MAX_CLIENTS *
NUM_MSG_PER_CLIENT), then stops the acceptor thread. It then collects all
messages from the linked list, prints them, frees the memory, and verifies the
count matches.

4. How are threads used in this sample code?
   The main thread creates an acceptor thread which accepts client connections.
For each client connection, the acceptor thread creates a new client thread to
handle receiving messages from that client.

Non-blocking Sockets:
1. How are sockets made non-blocking?
   Sockets are made non-blocking using fcntl(). First, F_GETFL gets the current
flags, then F_SETFL sets the flags with O_NONBLOCK added.

2. What sockets are made non-blocking?
   Both the server listening socket (sfd) in run_acceptor and each client
connection socket (cfd) in run_client are made non-blocking.

3. Why are these sockets made non-blocking? What purpose does it serve?
   Non-blocking sockets prevent threads from being stuck in blocking system
calls like accept() and read(). This allows the threads to periodically check
the run flag and exit gracefully when signaled. Without non-blocking sockets,
the threads would hang waiting for connections or data, preventing clean
shutdown.
*/
...
int main() {
  ...
  while (1) {
    ...
    uint32_t current_count = list_handle.count;
