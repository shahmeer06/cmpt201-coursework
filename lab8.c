...
static void add_word_counts_in_chunk(count_map_t *map, word_t *words,
  ...
  for (size_t i = 0; i < num_words; i++) {
    // Task 4: Lock before accessing shared hash table
    if (lock) {
    ...
    // Task 4: Unlock after accessing shared hash table
    if (lock) {
...
static count_map_t count_words_parallel(word_t *words, size_t num_words) {
  ...
  for (size_t i = 0; i < THREAD_COUNT; i++) {
    ...
    // Task 2: Prepare the arguments and launch the threads
    threads_args[i] =
  ...
  // Task 2: Wait for threads to finish
  for (size_t i = 0; i < THREAD_COUNT; i++) {
